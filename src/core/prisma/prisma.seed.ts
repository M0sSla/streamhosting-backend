...
const prisma = new PrismaClient({
    transactionOptions: {
        maxWait: 5000, // Максимальное время ожидания начала транзакции
        timeout: 12000, // Максимальное время выполнения транзакции
        isolationLevel: Prisma.TransactionIsolationLevel.Serializable // Уровень изоляции транзакции
    }
}) 
...
async function main() {
    try {
			...
			for (const username of USERNAMES) {
				...
				if (!userExists) {
					const createdUser = await tx.user.create({
						data: {
							email: `${username}@streamhosting.ru`,
        ...
        Logger.log("База данных отключена")
    }
}
